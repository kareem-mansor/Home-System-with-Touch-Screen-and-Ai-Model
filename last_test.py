...
class FaceRecognitionApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.video_label.setStyleSheet("border: 2px solid #333; background-color: #ddd;")
        layout.addWidget(self.video_label)

        register_button = QPushButton("Register Face", self)
        register_button.clicked.connect(self.register_user)
        register_button.setStyleSheet("background-color: #2E86C1; color: white; font-weight: bold;")
        ...
        recognize_button.setStyleSheet("background-color: #28B463; color: white; font-weight: bold;")
        layout.addWidget(recognize_button)

        delete_button = QPushButton("Delete User", self)
        delete_button.clicked.connect(self.delete_user)
        delete_button.setStyleSheet("background-color: #CB4335; color: white; font-weight: bold;")
        layout.addWidget(delete_button)

        bell_button = QPushButton("Bell", self)  # الزر الرابع
        bell_button.setStyleSheet("background-color: #F39C12; color: white; font-weight: bold;")
        bell_button.clicked.connect(self.on_bell_click)  # ربط الضغط بالوظيفة
        layout.addWidget(bell_button)

        container = QWidget()
        container.setLayout(layout)
        container.setStyleSheet("background-color: #F2F3F4;")  # خلفية الواجهة
    ...
    def recognition_loop(self):
        ...
        for encoding, (top, right, bottom, left) in zip(encodings, face_locations):
            ...
            for user_name, user_encoding in self.users.items():
                ...
                if True in matches:
                    name = user_name
                    print(4)  # Print 4 when a registered user is detected
                    self.ser.write(b'4')  # Send '4' to Arduino
                    break
    ...
    def on_bell_click(self):
        print(3)  # Print 3 in the terminal when Bell button is clicked
        
        # تشغيل الصوت باستخدام pygame
        pygame.mixer.music.load(r"C:\Users\karee\Desktop\facerecognition2\facerecognition2\big-bell-330719.wav")
